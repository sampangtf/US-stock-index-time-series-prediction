...
# train_x
# dtrain_x
VARselect(dtrain_x, lag.max = 20)
par(mfrow=c(2,1))
model=VARselect(dtrain_x)
...
aic.model.var=VAR(dtrain_x, p=AIC_ORDER)
bic.model.var=VAR(dtrain_x, p=BIC_ORDER)
...
cat("\n***************************************************")
cat("\n***************************************************")
summary(restrict(bic.model.var)$varresult$DJ)
...
cat("\n***************************************************")
cat("\n***************************************************")
normality.test(bic.model.var)
arch.test(bic.model.var)
serial.test(bic.model.var)
```
...
for (idx in 1:limits){
nfit = nrow(ts_data2) - n_test_quarter + (idx-1)*n_ahead
data = ts_data2[1:nfit,]
final.model = VAR(data, p=BIC_ORDER)
...
actuals2= ts_data2$DJ[(nrow(ts_data2)-35):nrow(ts_data2)]
...
dj_plot=unlist(ts_data2[,2])
  #diff(ts_data,1)[,2]
dmin=unclass(ts_data2[,2])
  #unlist(ts_data2[,2])
  #unlist(data[(n-20):n,3])
ymin = min(c(dmin,DJ.fcst))
ymax = max(c(dmin,DJ.fcst))
...
NEW_PRED=DJ.fcst
NEW_PRED[1]=0
NEW_PRED[2]=test_x[1,1]+DJ.fcst[1]
for(i in 3:(length(DJ.fcst)+1)){
  NEW_PRED[i]=NEW_PRED[(i-1)]+DJ.fcst[(i-1)]
}

NEW_PRED=NEW_PRED[2:length(NEW_PRED)]
#length(NEW_PRED)
```
...
plot(Date_full[(n-50):n], dj_plot[(n-50):n],type="l", xlab="Time", ylab="DJ Index")
points(Date_full[(nfit+1):n],NEW_PRED,col="red",lwd=2)
